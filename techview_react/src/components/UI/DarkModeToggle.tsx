...
const DarkModeToggle = () => {
  ...
  useEffect(() => {
    ...
    // Ensure Tailwind processes the classes
  ...
  return (
    ...
    >
      {/* Moon Icon (Light Mode - shows when in light mode) */}
      <svg
        className={`w-5 h-5 transition-all duration-300 absolute ${
          !isDark ? 'opacity-100 scale-100 text-gray-800' : 'opacity-0 scale-0 text-gray-800'
        }`}
        fill="currentColor"
        viewBox="0 0 24 24"
      >
      ...
      {/* Sun Icon (Dark Mode - shows when in dark mode) */}
      <svg
        className={`w-5 h-5 transition-all duration-300 absolute ${
          isDark ? 'opacity-100 scale-100 text-gray-800' : 'opacity-0 scale-0 text-gray-800'
        }`}
